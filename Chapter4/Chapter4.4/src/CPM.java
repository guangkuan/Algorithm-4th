 ...
 *  Execution:    java CPM < input.txt 在run configuration 中common导入txt文件完整路径
...
//            StdOut.printf("%.1f ", duration);
            G.addEdge(new DirectedEdge(source, i, 0.0));
            G.addEdge(new DirectedEdge(i, i+n,    duration));
            G.addEdge(new DirectedEdge(i+n, sink, 0.0));
            StdOut.printf("--> %d -> %d 0.0, %d -> %d %.1f, %d -> %d 0.0,\n", source, i, i, i+n, duration, i+n, sink);

            // precedence constraints
            for (int j = 1; j < a.length; j++) 
            {
                int precedent = Integer.parseInt(a[j]);
                G.addEdge(new DirectedEdge(n+i, precedent, 0.0));
                StdOut.printf("==> %d -> %d 0.0, ", n+i, precedent);
            }
            StdOut.printf("\n");
        }
