...
public class AcyclicLP {
    ...
    public AcyclicLP(EdgeWeightedDigraph G, int s) 
    {
        ...
        {
        	StdOut.printf("add %d to the tree, ", v);
    ...
    private void relax(DirectedEdge e) 
    {
        ...
        {
        	if(edgeTo[w] != null)
        		StdOut.printf("%d -> %d is invalid, ", edgeTo[w].from(), edgeTo[w].to());
            distTo[w] = distTo[v] + e.weight();
            edgeTo[w] = e;
            StdOut.printf("add %d -> %d to the tree, distance is %.2f. ", v, w, distTo[w]); 
        } 
        else
        {
        	StdOut.printf("%d -> %d is invalid, ", e.from(), e.to());
        }
    }
